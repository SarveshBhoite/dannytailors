...
const InternalVerticalBorder = ({ className = "" }: { className?: string }) => (
  <div className={`pointer-events-none absolute top-0 right-0 h-full z-10 ${className}`} style={{ width: borderConfig.width }}>
...
const InternalHorizontalBorder = ({ className = "" }: { className?: string }) => (
  <div className={`pointer-events-none absolute bottom-0 left-0 w-full z-10 ${className}`} style={{ height: borderConfig.width }}>
    <svg className="h-full w-full overflow-visible">
      <line 
...
const boughtTogether: Product[] = [
    { id: 1, title: "Timeless A-line Evening Dress", price: "$109.99", fit: "Ankle-length", category: "Womenswear", image: "/shop-now/chefcoat/prod1.svg" },
    { id: 2, title: "Floral Bloom Maxi Dress", price: "$54.99", fit: "Slim Fit", category: "Womenswear", image: "/shop-now/chefcoat/prod2.svg" },
    { id: 3, title: "Elegant Evening Gown", price: "$89.99", fit: "Flowing skirt", category: "Womenswear", image: "/shop-now/chefcoat/prod3.svg" },
];

const recommended: Product[] = [
    { id: 4, title: "Wide-Brim Bucket Hat", price: "$69.99", fit: "Any face shape", category: "Accessories", image: "/shop-now/pants/prod1.svg" },
    { id: 5, title: "Sophisticate Sun Hat", price: "$24.99", fit: "One size fits all", category: "Accessories", image: "/shop-now/pants/prod2.svg" },
    { id: 6, title: "Timeless Fedora", price: "$79.99", fit: "Any face shape", category: "Accessories", image: "/shop-now/pants/prod3.svg" },
];
    ...
    <section className="w-full max-w-[1280px] mx-auto mt-20 mb-20 px-4 xl:px-0">
        ...
        <div className="relative bg-[#000a23] rounded-[16px] overflow-hidden">
            ...
            <div className="relative flex flex-row items-center justify-between px-[23px] py-[23px]">
                ...
                <h2 className={`${montserrat.className} text-[18px] md:text-[23px] font-medium text-white uppercase`}>
                    BOUGHT TOGETHER FREQUENTLY
                </h2>
                <Link href="/products" className="relative flex items-center justify-center gap-2 w-[113px] h-[48px] bg-[#000A23] rounded-[9px] group/btn">
                    <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 113 48" fill="none" preserveAspectRatio="none">
                        <rect x="0.5" y="0.5" width="112" height="47" rx="9" stroke="#8B744B" strokeWidth="0.76" strokeDasharray="6 3"/>
                    </svg>
                    ...
                    <span className={`${montserrat.className} text-[13.8px] text-white`}>View All</span>
            ...
            <div className="relative grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                ...
                {boughtTogether.map((product, index) => (
                    <article key={product.id} className="relative flex flex-col p-[23px] gap-[23px]">
                        
                        {/* VERTICAL BORDER: Right side (Hidden on last column) */}
                        <InternalVerticalBorder className={`
                            hidden md:block 
                        ...
                        <div className="relative w-full h-[296px] bg-[#D9D9D9] rounded-t-[38px] overflow-hidden">
                            <Image src={product.image} alt={product.title} fill className="object-cover object-top transition-transform duration-700 hover:scale-105" />
                        </div>
                        ...
                        <div className="flex flex-col gap-[15px]">
                            <div className="flex justify-between items-center h-[48px]">
                                <div className="flex items-center justify-center bg-[#212F52] rounded-[76px] px-[12px] py-[7px] h-[36px]">
                                    <span className={`${montserratalter.className} text-[13.8px] text-[#D2B589] whitespace-nowrap`}>
                                        {product.category}
                                    </span>
                                </div>
                                <button className="relative flex items-center justify-center gap-1 w-[121px] h-[48px] bg-[#000A23] rounded-[9px] group/btn">
                                    ...
                                    <div className="absolute bottom-0 right-0 w-[12px] h-[12px] border-b border-r border-[#D2B589] rounded-br-[9px]" />
                                    <div className="absolute bottom-0 left-0 w-[12px] h-[12px] border-b border-l border-[#D2B589] rounded-bl-[9px]" />
                                    <span className={`${montserrat.className} text-[13.8px] font-normal text-white`}>Shop Now</span>
                            ...
                            <div className="flex items-center gap-[15px]">
                                <div className="flex items-center gap-[6px]">
                                    <span className={`${montserratalter.className} text-[13.8px] text-[#81807E]`}>Fit</span>
                                    <div className="w-[3px] h-[3px] bg-[#4D4D4D] rounded-full" />
                                    <span className={`${montserratalter.className} text-[15.3px] font-medium text-[#CCCCCC] whitespace-nowrap`}>{product.fit}</span>
                                </div>
                                <div className="flex items-center gap-[6px]">
                                    <span className={`${montserratalter.className} text-[13.8px] text-[#81807E]`}>Price</span>
                                    <div className="w-[3px] h-[3px] bg-[#4D4D4D] rounded-full" />
                                    <span className={`${montserratalter.className} text-[15.3px] font-medium text-[#CCCCCC]`}>{product.price}</span>
                                </div>
                            </div>
                        </div>
                    </article>
