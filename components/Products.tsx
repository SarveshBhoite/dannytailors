...
// Figma dash spec
const DASH = 12.270675659179688; // px
const GAP = 4.6015; // px
const STROKE = 1.53; // px
...
const collections: Collection[] = [
    ...
    // NEW: Uniform Collection after Aprons
    {
        id: 4,
        ...
        <section className="mt-0">
            <div className="relative px-2 pb-12 pt-5 md:px-8 lg:px-2">
                ...
                <div className="relative overflow-hidden rounded-[15.34px] px-6 py-8 md:px-10">
                    ...
                    {/* FAQ grid + vertical + horizontal dividers */}
                    <div className="relative mt-8">
                        {/* vertical dashed line between columns (md+) */}
                        ...
                        {/* Desktop/tablet rows with separators between rows */}
                            ...
                            {rows.map((row, rowIndex) => (
                                <div key={`row-${rowIndex}`} className="relative grid grid-cols-2 gap-6">
                                    {row.map((faq) => (
                                        <div
                                            key={faq.id}
                                            className="relative overflow-hidden rounded-[15.34px] px-6 py-10"
                                        >
                                    ...
                                    {/* horizontal dashed line between this row and the next one */}
                                    {rowIndex < rows.length - 1 && (
                                        <div
                                            className="pointer-events-none absolute left-0 right-0 hidden md:block"
                                            aria-hidden
                                            style={{ bottom: `-${HALF_ROW_GAP}px` }}
                                        >
                                            <div
                                                className="w-full"
                                                style={{
...
export default function Products() {
    return (
        <div className={`${montserrat.className} min-h-screen bg-[#000a23] px-4 pb-12 pt-24 md:px-8 lg:px-16`}>
            {/* OUTER CARD – dashed SVG border wraps hero + all collections + FAQ */}
            <div className="relative mx-auto max-w-7xl overflow-hidden">
                {/* Outer dashed frame (SVG) */}
                <svg
                    className="pointer-events-none absolute inset-0 h-full w-full"
                ...
                <div className="rounded-[15.34px] bg-[#000a23]">
                    ...
                    <div className="relative h-[260px] overflow-hidden md:h-[360px] lg:h-[450px]">
                        ...
                        <div className="pointer-events-none absolute inset-0 ml-6 flex items-center justify-center pt-70">
                            <h1 className="text-center text-4xl font-semibold tracking-[0.2em] text-white md:text-6xl lg:text-[9.6rem]">
                                SHOP&nbsp;NOW
                            </h1>
                        </div>

                        {/* navy curved bump under button */}
                        <div className="pointer-events-none absolute bottom-0 left-1/2 z-[5] h-14 w-40 -translate-x-1/2 rounded-t-[16px] bg-[#000a23]" />

                        {/* small dashed SHOP NOW button at bottom center */}
                            ...
                            <button className="relative inline-flex items-center gap-2 rounded-[8.18px] px-[20px] py-[12px] text-[0.85rem] text-[#ffffff]">
                                ...
                                >
                                    <rect
                                        ...
                                        stroke="#AE9B84"
                                        strokeWidth="1.02"
                                        strokeDasharray="16.3609 6.13533"
                                    />
                                </svg>
                            </button>
                        </div>
                    ...
                    {collections.map((collection, index) => (
                        <section key={collection.id} className={index === 0 ? "mt-0" : "mt-0"}>
                            ...
                            {/* PRODUCT LIST AREA FOR THIS COLLECTION */}
                            <div className="relative px-4 pb-8 pt-6 md:px-6 lg:px-8">
                                {/* dashed separators under/over the grid (Figma spec) */}
                                ...
                                {/* vertical dashed separators aligned to columns (Figma spec) */}
                                <div className="pointer-events-none absolute top-0 bottom-0 left-4 right-4 md:left-6 md:right-6 lg:left-8 lg:right-8" aria-hidden>
                                    {/* 2 columns (sm to lg-) */}
                                    <div
                                        className="absolute inset-y-0 hidden sm:block lg:hidden -translate-x-1/2"
                                        style={{
                                            left: "50%",
                                            width: `${STROKE}px`,
                                            backgroundImage: patternY,
                                    ...
                                    {/* 3 columns (lg+) */}
                                    <div
                                        className="absolute inset-y-0 hidden lg:block -translate-x-1/2"
                                        style={{
                                            left: "33%",
                                            width: `${STROKE}px`,
                                            backgroundImage: patternY,
                                        ...
                                        >
                                            ...
                                            <div className="relative flex flex-1 flex-col justify-between px-6 py-5">
                                                ...
                                                <div className="mb-4 flex items-center justify-between">
                                                    <span
                                                        className={`${montserratalter.className} rounded-full border border-[#263252] bg-[#212F52] px-4 py-1 text-[0.81rem] tracking-[0.1em] text-[#D2B589]`}
                                                    >
                                                        {product.badge}
                                                    </span>
                                                ...
                                                {/* title */}
                                                <h3 className="text-sm font-semibold text-white md:text-[1.14rem]">
                                                    {product.title}
                                                </h3>

                                                {/* fit + price */}
                                                <div
                                                    className={`${montserratalter.className} mt-4 flex flex-wrap items-center gap-2 text-[0.75rem] text-[#81807E]`}
                                                >
                                                    <span>
                                                        Fit -{" "}
